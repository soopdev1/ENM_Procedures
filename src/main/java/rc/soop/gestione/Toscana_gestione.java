...
public class Toscana_gestione {
    ...
    public List<String> ore_convalidateAllievi(String sql_adhoc) {
        ...
        String sql1;
        if (sql_adhoc == null) {
            sql1 = "SELECT a.idallievi FROM allievi a WHERE a.id_statopartecipazione IN (15,16,17)";
        } else {
            sql1 = sql_adhoc;
        }
...
//            List<String> presenzeconv = ore_convalidateAllievi();
                ...
                try (Statement st0 = db1.getConnection().createStatement(); ResultSet rs0 = st0.executeQuery(sql0)) {
                    while (rs0.next()) {
                        ...
                        try (PreparedStatement ps13 = db1.getConnection().prepareStatement(sql13)) {
                            ps13.setString(1, statopartecipazione);
    ...
    public void report_docenti_GG1(boolean save) {
        try {
...
//                        String gf = rs1.getString("lm.gruppo_faseB");
    ...
    public void attestati_UD(int iduser) {
        ...
        String sql0 = "SELECT a.cognome,a.idallievi,a.idprogetti_formativi,a.codudok_A,a.codudok_B,a.gruppo_faseB "
                + "FROM allievi a WHERE a.orec_totali < 70 AND a.id_statopartecipazione NOT IN ('16','17') "
                + "AND a.idprogetti_formativi IN (SELECT p.idprogetti_formativi FROM progetti_formativi p WHERE p.stato IN "
                + "('F','DVB','IV','CK','EVI','CO'))  AND a.idallievi NOT IN (SELECT d.idallievo FROM documenti_allievi d WHERE d.tipo=23);";
            ...
            try (ResultSet rs = ps.executeQuery()) {
                ...
                while (rs.next()) {
                    ...
                    if (!A_UD.toString().equals("") || !B_UD.toString().equals("")) {
                        ...
                        if (iduser > 0) {
                            log.log(Level.INFO, "{0} RILASCIATO.", at1.getPath());
                        } else {
                            String ins = "INSERT INTO documenti_allievi (path,idallievo,tipo,deleted) VALUES (?,?,?,?)";
    ...
    public void ore_ud(String sql_adhoc) {
        ...
        String sql;
        if (sql_adhoc == null) {
            sql = "SELECT a.idallievi FROM allievi a WHERE a.id_statopartecipazione IN (15,16,17)";
        } else {
            sql = sql_adhoc;
        }
        ...
        try (Statement st0 = db0.getConnection().createStatement(); ResultSet rs0 = st0.executeQuery(sql0)) {
            while (rs0.next()) {
                ...
                if (MOD_codici.size() == 4 && MOD_codici.get(0).startsWith("A")) {
                ...
                } else if (MOD_codici.size() == 4 && MOD_codici.get(0).startsWith("B-1")) {
                    ...
                    MOD_ore.put(ud11_2, "5");
                    MOD_ore.put(ud11_3, "5");
                } else if (MOD_codici.size() == 4 && MOD_codici.get(0).startsWith("B-3")) {
                    String ud11_1 = "B-3A_B-3B";
                    String ud11_2 = "B-3B_B-3C";
                    String ud11_3 = "B-3C_B-3D";
                    MOD_codici.add(ud11_1);
                    MOD_codici.add(ud11_2);
                    MOD_codici.add(ud11_3);
                    MOD_ore.put(ud11_1, "5");
...
//                    System.out.println("rc.soop.gestione.Toscana_gestione.ore_ud( ) "+sql2_F0);
                    String datainizio;
                    String datafine;
...
//                            System.out.println("OK) " + nomemodulo + " --- " + res);
            ...
            try (Statement st3 = db0.getConnection().createStatement(); ResultSet rs3 = st3.executeQuery(sql3)) {
                while (rs3.next()) {
                    ...
                    try (PreparedStatement ps3A = db0.getConnection().prepareStatement(upd3)) {
                        ...
                        boolean es = ps3A.executeUpdate() > 0;
...
//                System.out.println("rc.soop.gestione.Toscana_gestione.ore_ud() "+unit.toString());
...
//        new Toscana_gestione(false).report_allievi(false);
//    }
}
