...
public class Create {
    ...
    public static void crearegistri(boolean testing, boolean all) {
...
//        list_id.add(13);
        try {
            ...
            if (all) {
                sql0 = "SELECT DISTINCT(mp.id_progettoformativo) "
                        + "FROM lezioni_modelli lm, modelli_progetti mp , progetti_formativi pf "
                        + "WHERE mp.id_modello=lm.id_modelli_progetto AND mp.id_progettoformativo=pf.idprogetti_formativi "
                        + "AND lm.tipolez='F' AND pf.stato<>'CO' ORDER BY mp.id_progettoformativo";
            }
